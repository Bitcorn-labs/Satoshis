...
shared ({ caller = _owner }) actor class Token(
  ...
  let DKPFeeE8s = 100_000;
  let DKPFeeE12s = 100_000_0000; // 8 decimals -> 12 decimals
  let DPWFeeE12s = 1250;
  ...
  public shared ({ caller }) func withdraw(subaccount : ?[Nat8], amount : Nat) : async Result.Result<(Nat, Nat), Text> {
    ...
    //let returnAmount = (amount - DKPFeeE12s) * 10 ^ -4 (or something)
    ...
    let result = try {
      await DKPLedger.icrc1_transfer({
        ...
        created_at_time = ?time64();
        amount = amount - DKPFeeE8s; // keep this amount as part of the transfer fee to keep our DKP from being drained.
      });
    } catch (e) {
      //put back
